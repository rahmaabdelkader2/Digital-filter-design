...
const s = (p5_inst) => {
    ...
    class Zero extends Point {
        ...
        draw(size = 10, fill = '#fff', picked = false) {
            p5_inst.push()
    ...
    class FilterPlane {
        ...
        getZerosPoles(max) {
            ...
            console.log("$$$$$")
            var zeros_poles={'zeros':zerosPositions,'poles':polesPositions}
            $.ajax({
                url: "/unitcircle",
                type: "POST",
                contentType: "application/json",
                data: JSON.stringify(zeros_poles),
                success: function (response) {
                    console.log("done response")
                    Plotly.newPlot('filter-mag-response', [{
                        x: JSON.parse(response)['frequency'],
                        y: JSON.parse(response)['magnitude'],
                        type: 'scatter',
                    }]);  
...
const customeBtn = document.getElementById ("myfile");
